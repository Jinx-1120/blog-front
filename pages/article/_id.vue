  ...
  <div class="article-list" :class="{'mobile-article': mobileLayout}">
    ...
    <div class="article-cont">
      ...
      <div class="meta">
        ...
        <span class="view">阅读 {{ article.viewCount || 0 }}</span>
        <span class="view">喜欢 {{ article.fabulous || 0 }}</span>
        <!-- <span class="comment">评论 {{ article.meta.comments }}</span> -->
        <span class="count">
          <!-- <span class="disqus-comment-count" :data-disqus-identifier="article._id"></span> -->
        </span>
      </div>
      <div class="article-thumb" v-if="article.coverImg">
...
export default {
  ...
  methods: {
    async like () {
      ...
      if (res.code !== 200) alert(`喜欢文章失败：${res.message}`)
      else {
        let fabulous = Number(this.article.fabulous) + 1 || 1
        this.$store.commit('article/updateFabulous', fabulous)
